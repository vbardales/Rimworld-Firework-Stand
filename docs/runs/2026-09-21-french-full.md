...
- **Evidence, on disk and ignored by git, trimmed on 2026-09-23 to the most relevant:** `Tests/Pickle/runs/2026-09-21-french-full/` keeps `summary.json`, `junit.xml`, 8 stills and the 5 films (2.9 MB, down from 50 MB). Deleted: the log, the messages, the derived contact sheets and half-size copies, the 6 stills polluted by the film frame (the reshaped suite regenerates them) and 2 redundant ones (the empty stand and the stand selected, which the inspect-pane stills repeat). The 8 kept stills were converted from png to jpeg (about 0.2 MB each instead of 3.5 MB, still fully readable at 1920x1080). The 16 stills and 5 films were all opened before the trim.
